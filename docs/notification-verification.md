...
- [ ] **Welcome copy on cold start.** Reload window, immediately open the Salesforce Extensions Manager activity-bar view.
  _Expected:_ the Groups and Dependencies views show the friendly descriptive welcome copy — NOT VSCode's built-in *"There is no data provider registered that can provide view data."* message, and no action buttons. Welcome disappears once the tree populates.
