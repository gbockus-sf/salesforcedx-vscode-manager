...
- **Friendly welcome copy for the Groups and Dependencies views on cold start.** Replaces VSCode's built-in *"There is no data provider registered that can provide view data."* message with localized copy + command-link buttons (`Apply a Group`, `Browse Salesforce Extensions`, `Run Dependency Check`). The welcome content also doubles as a first-run affordance on fresh installs where the managed-extension list is still loading.
