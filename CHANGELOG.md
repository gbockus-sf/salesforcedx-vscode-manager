...
- **Salesforce CLI update indicator in the Dependencies view.** When the installed `sf` is older than the latest stable release on `developer.salesforce.com`, the Salesforce CLI row shows an `$(arrow-circle-up)` icon and an "update → v<latest>" badge; the tooltip gives the upgrade command hint (`sf update` / `npm install -g @salesforce/cli`). Offline / `updateCheck=never` / first-run paths stay quiet — any missing piece suppresses the badge. New `CliVersionService` caches the probe for 1 h.
